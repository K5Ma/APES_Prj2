...
void LoadCell_Task(void *pvParameters)
{
    ...
    if(LC_Error == false)
    {
        ...
        #if     LC_DEBUG_PRINTF
            Log_Msg(T_LoadCell, "INFO", "LoadCell Initialized", LOCAL_ONLY);
            Log_Msg(T_LoadCell, "INFO", "Starting LC Normal Operation", LOCAL_ONLY);
        #endif
    }
    else
    {
        ...
        #if     LC_DEBUG_PRINTF
            Log_Msg(T_LoadCell, "CRITICAL", "LoadCell Initialization Failed", LOCAL_ONLY);
        #endif
    }
...
//		                    #endif
		                    ...
		                    #if     LC_DEBUG_PRINTF
		                            if(LC_Error == false)  Log_Msg(T_LoadCell, "INFO", "LoadCell is Back Online", LOCAL_ONLY);
		                            else
									{
		                            	if(LC_counter >= 10)
		                            	{
...
//		                    LC_Print("\nChecking LoadCell Status...");
		                    ...
		                    else     Log_Msg(T_LoadCell, "CRITICAL", "LoadCell is Offline", LOCAL_ONLY);
		            #endif
		        }
