...
void NFC_Task(void *pvParameters)
{
  ...
  //  NFC_UART_Init();
    ...
    if(NFC_Error == true)
    {
        ...
        #if     NFC_DEBUG_PRINTF
            Log_Msg(T_NFC, "CRITICAL", "NFC Module Initialization Failed!", LOCAL_ONLY);
            Log_Msg(T_NFC, "CRITICAL", "NFC Module is Critical... Exiting Task...", LOGGER_AND_LOCAL);
        #endif
    }
    else
    {
...
//                    Log_Msg(T_NFC, "INFO", "Checking NFC Module State...", LOCAL_ONLY);
        ...
        else if(NFC_Startup_Mode == true)
        {
            ...
            if(timeout >= NFC_Online_Test_Timems)
            {
                timeout = 0;
                #if     NFC_DEBUG_PRINTF
                    Log_Msg(T_NFC, "CRITICAL", "NFC Module is Offline", LOCAL_ONLY);
                #endif
